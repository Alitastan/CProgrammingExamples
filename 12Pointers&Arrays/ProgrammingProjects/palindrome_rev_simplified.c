...
int main (void)
{
    char message_buffer[MESSAGE_LEN], *position, input, *reverse_input = message_buffer;
    bool is_palindrome = true;

    printf("Enter a message: ");

    for (position = message_buffer; position < message_buffer + MESSAGE_LEN; )
    {
    ...
    for ( ; position >= message_buffer; position--, reverse_input++)
    {
